...
[ -f "$HOME/.claude-fleet/lib/event-bus.sh" ] && EVENT_BUS_ENABLED=false source "$HOME/.claude-fleet/lib/event-bus.sh"
...
_GS_FILE="$HOME/.claude-fleet/state/sessions/$SESSION_ID/graceful-stop"
...
if [ -n "$INJECTIONS" ] && [ -f "$INJECTIONS" ]; then
  ...
  CONTEXT=$(echo "$INPUT" | python3 "$HOME/.claude-fleet/lib/py/policy_match.py" \
...
if [ "$INBOX_ENABLED" = "true" ]; then
  ...
  if [ -z "$INBOX_CONTEXT" ]; then
    ...
    LEGACY_CONTEXT=$(python3 "$HOME/.claude-fleet/lib/py/inbox_scan.py" \
