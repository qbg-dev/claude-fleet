...
server.registerTool(
  "add_hook",
  {
    ...
    inputSchema: {
      ...
      ]).describe("Which hook event to fire on. Common: Stop (blocks session exit), PreToolUse (fires before tool call), PreCompact (before context compaction), PostCompact (after compaction — inject 'keep going' prompts), StopFailure (API error ended turn), Elicitation (MCP server requests user input)"),
