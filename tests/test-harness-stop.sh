...
trap 'rm -rf "$TMPDIR_TEST"; rm -rf "${HARNESS_STATE_DIR:-$HOME/.boring/state}/harness-runtime/test-stop-harness"' EXIT

# ── Test: harness list shows harnesses ──
LIST_OUTPUT=$(bash ~/.boring/bin/harness.sh list 2>&1)
assert "list shows 'Harnesses'" "Harnesses" "$LIST_OUTPUT"

# ── Test: harness-stop.sh wrapper redirects to harness.sh stop ──
WRAPPER_OUTPUT=$(bash ~/.boring/bin/harness-stop.sh nonexistent 2>&1 || true)
assert "wrapper calls harness stop" "no progress file" "$WRAPPER_OUTPUT"

# ── Test: harness help shows usage ──
HELP_OUTPUT=$(bash ~/.boring/bin/harness.sh help 2>&1 || true)
assert "help shows usage" "harness" "$HELP_OUTPUT"

# ── Test: missing harness returns error ──
MISSING_OUTPUT=$(bash ~/.boring/bin/harness-stop.sh nonexistent-harness-xyz 2>&1 || true)
...
MOCK_MANIFEST_DIR="$HOME/.boring/harness/manifests/test-stop-harness"
...
HARNESS_SESSION_REGISTRY="$MOCK_REGISTRY" bash ~/.boring/bin/harness-stop.sh test-stop-harness 2>&1 || true
...
assert_file_exists "stop creates harness stop flag" "${HARNESS_STATE_DIR:-$HOME/.boring/state}/harness-runtime/test-stop-harness/stop-flag"

# ── Test: harness-dispatch.sh has harness-level escape check ──
DISPATCH_SCRIPT="$HOME/.boring/hooks/harness-dispatch.sh"
...
EOF
STOP_OUTPUT=$(HARNESS_SESSION_REGISTRY="$MOCK_REGISTRY" bash ~/.boring/bin/harness-stop.sh test-stop-harness 2>&1 || true)
...
rm -rf "${HARNESS_STATE_DIR:-$HOME/.boring/state}/harness-runtime/test-stop-harness"
