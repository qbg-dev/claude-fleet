...
# Run: bash ~/.boring/tests/test-hooks.sh
set -euo pipefail
...
cleanup() {
  ...
  rm -f "${HARNESS_STATE_DIR:-$HOME/.boring/state}/activity/claude_activity_miniapp-chat.jsonl.test"
  rm -f "${HARNESS_STATE_DIR:-$HOME/.boring/state}/logs/deploy-mutations.log"
...
RESULT=$(echo "{\"session_id\":\"$MOCK_SESSION\",\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"echo y | ./scripts/deploy-prod.sh\"}}" \
  | PROJECT_ROOT="$PROJECT_ROOT" bash "$HOME/.boring/hooks/admission/deploy-mutator.sh" 2>/dev/null)
...
RESULT=$(PROJECT_ROOT="$PROJECT_ROOT" HARNESS="miniapp-chat" bash "$HOME/.boring/hooks/admission/task-readiness.sh" "fix-send-button" 2>/dev/null || true)
...
RESULT=$(PROJECT_ROOT="$PROJECT_ROOT" HARNESS="miniapp-chat" bash "$HOME/.boring/hooks/admission/task-readiness.sh" "_test-task" 2>/dev/null || true)
...
EOF
assert_exit "passes with complete artifact" 0 env PROJECT_ROOT="$PROJECT_ROOT" HARNESS="miniapp-chat" bash "$HOME/.boring/hooks/admission/task-readiness.sh" "_test-task"
...
echo "{\"session_id\":\"$MOCK_SESSION\",\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"ls -la\"}}" \
  | PROJECT_ROOT="$PROJECT_ROOT" bash "$HOME/.boring/hooks/operators/activity-logger.sh" 2>/dev/null
...
RESULT=$(FILE_PATH="$TEMP_TSX" bash "$HOME/.boring/hooks/operators/checks.d/no-inline-styles.sh" 2>/dev/null || true)
...
RESULT=$(FILE_PATH="$TEMP_TS" bash "$HOME/.boring/hooks/operators/checks.d/no-mock-data.sh" 2>/dev/null || true)
assert "catches mock data" "mock/placeholder" "$RESULT"
rm -f "$TEMP_TS"

# Test 21: Ignores non-ts files
RESULT=$(FILE_PATH="/tmp/readme.md" bash "$HOME/.boring/hooks/operators/checks.d/no-inline-styles.sh" 2>/dev/null || true)
...
RESULT=$(echo "{\"session_id\":\"$MOCK_SESSION\",\"tool_input\":{\"file_path\":\"$PROJECT_ROOT/claude_files/miniapp-chat-progress.json\"}}" \
  | PROJECT_ROOT="$PROJECT_ROOT" bash "$HOME/.boring/hooks/operators/progress-validator.sh" 2>/dev/null || true)
TOTAL=$((TOTAL + 1))
...
_RUNTIME_DIR="${HARNESS_STATE_DIR:-$HOME/.boring/state}/harness-runtime/miniapp-chat"
...
# ═════════════════════════════════════════════════════════════════════
# symlink verification (project hooks → ~/.boring/hooks/)
# ═════════════════════════════════════════════════════════════════════
...
assert_symlink "progress-validator.sh symlink" \
  ...
  "$HOME/.boring/hooks/operators/progress-validator.sh"
...
RESULT=$(echo "{\"session_id\":\"$MOCK_SESSION_CYCLE\"}" \
  | PROJECT_ROOT="$CYCLE_TEST_DIR" CYCLE_GATE_ENABLED=true AGENT_DISCOVERY_ENABLED=false \
    bash "$HOME/.boring/hooks/harness-dispatch.sh" 2>/dev/null)
...
rm -f "${HARNESS_STATE_DIR:-$HOME/.boring/state}/activity/claude_activity_miniapp-chat.jsonl"
