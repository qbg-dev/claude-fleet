...
export async function runCreate(
  ...
  // 8b. Codex-specific setup: .codex/config.toml + global trust + AGENTS.md
  if (runtime === "codex") {
...
- **Branch**: \`worker/${name}\`
- **Runtime**: codex
- **Fleet CLI**: \`fleet mail\`, \`fleet state\`, \`fleet checkpoint\`, \`fleet round-stop\`, \`fleet hook\`

## Key Commands

\`\`\`
# Fleet Mail
fleet mail inbox                         # check inbox
fleet mail send <to> "<subject>" "<body>"
fleet round-stop "<message>"             # end work cycle
...
- After changes: report to chief-of-staff via \`fleet mail send\`

## Workflow

1. \`fleet mail inbox\` — check messages first
2. \`git fetch origin && git rebase origin/main\`
3. Work on mission
4. Commit frequently with specific file staging
5. \`fleet round-stop "message"\` when done with a cycle
`;
